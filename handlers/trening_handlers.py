...
async def start_kontrol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        text="Вы зашли в план подготовки к контрольной. Напиши предмет, тему и всё, что посчитаете нужным. \nПример: биология, клетки",
        reply_markup=markup,
    )
    return KONTROL
        ...
        input=[
            {
                ...
                "content": f"Ты ассистент школьника {context.user_data['class_user']} класса, {topic_predmet_kontrol} вот тема и предмет по которому будет контрольная ученика."
                           ...
                           f'В ответе дай список вопросов в формате:вопрос1;вопрос2.НЕ ВКОЕМ СЛУЧАЕ НЕ ПИШИ ПУСТЫХ ПУНКТОВ. Пиши только вопросы, ничего кроме них не пиши.В ОТВЕТЕ НЕ БОЛЬШЕ ЧЕМ 1024 СИМВОЛОВ ПИШИ.',
            },
        ],
    )
        ...
        input=[
            {
                "role": "developer",
                "content":  
                             f"Ты ассистент школьника {context.user_data['class_user']} класса,дай ответы на эти вопросы: {answer_text}."
                             f'Пиши только ответы(СТАРАЙСЯ ЧТОБЫ БЫЛО ПОНЯТНО, НО КРАТКО), ничего кроме них не пиши.Ответы пиши в формате 1. ####, новая строка 2. #### и так далее.'#, номер 1 вопроса не пиши но сам текст пиши.'
                             f'В ОТВЕТЕ НЕ БОЛЬШЕ ЧЕМ 1024 СИМВОЛОВ ПИШИ.Пиши грамотно, без любых ошибок.'
                             f'Ответы должны быть оформлены в скрытом формате, например: 1.<tg-spoiler>текст ответа</tg-spoiler>.',
